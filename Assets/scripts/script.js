...
var gameEl = document.getElementById('hidden-word');
...
function keydownAction(event) {
  ...
  for (var i = 0; i < randomWord.length; i++) {
    if (keyPressed.includes(randomWord.charAt(i))) {
    gameEl.textContent += randomWord.charAt(i); 
    } else {
      gameEl.textContent += " _ ";
    };
  };
  if (gameEl.textContent === randomWord) {
    gameStatus = "won";
  }
};
...
button.addEventListener("click", function() {
  ...
  gameEl.textContent = " _ ".repeat(randomWord.length);
  countdown();
});